...
public class Weapon : MonoBehaviour {
	...
	void Effect( Vector3 hitPos, Vector3 normal)
	{
	    ...
	    {
            Transform hitParticle = Instantiate(hitPrefab, hitPos, Quaternion.FromToRotation(Vector3.right, normal));
	        Destroy(hitParticle.gameObject, 1f);
        }

		// muzzle flash
		Transform clone = Instantiate(MuzzleFlash, firePoint.position, firePoint.rotation);
        clone.parent = firePoint;
        float size = Random.Range(0.6f, 0.9f);
		clone.localScale = new Vector3(size, size, size);
		Destroy(clone.gameObject, 0.02f);

        camShake.Shake(camShakeAmount,camShakeLenght);
